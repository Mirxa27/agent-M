...
class DropboxOAuthService {
  ...
  constructor() {
    if (!config.oauth.credentials.dropbox.clientId || !config.oauth.credentials.dropbox.clientSecret) {
  ...
  async exchangeCodeForTokens(code: string): Promise<OAuthTokens> {
    const response = await this.dbx.getAccessTokenFromCode(
      config.oauth.redirectUris.dropbox,
      code
    );
    ...
    return {
      ...
      expiresAt: response.result.expires_in ? 
        new Date(Date.now() + response.result.expires_in * 1000) : undefined,
...
export class OAuthService {
   ...
   */
  async completeOAuthFlow(code: string, state: string): Promise<{ userId: number; credential: any }> {
    // Verify state
  ...
  isOAuthSupported(service: string): boolean {
    const supportedServices = ['google', 'gmail', 'google_drive', 'google_calendar', 'microsoft', 'outlook', 'onedrive', 'dropbox'];
