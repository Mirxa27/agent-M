...
export class PaymentService {
  ...
  async createPaymentSession(
    ...
    try {
      console.log(
        "Creating MyFatoorah payment session for user:",
        userId,
        "plan:",
        planId,
      );
      const response = await myfatoorahClient.post(
        "/v2/InitiatePayment",
        payload,
      );
      ...
      console.log(
        "MyFatoorah payment session created successfully:",
  ...
  async verifyPayment(paymentId: string): Promise<{
    ...
    try {
      ...
      if (paymentData.InvoiceStatus === "Paid") {
        ...
        console.log(
          "MyFatoorah payment not completed. Status:",
