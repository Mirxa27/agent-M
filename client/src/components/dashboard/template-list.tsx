...
export default function TemplateList({ templates, isLoading, onUseTemplate }: TemplateListProps) {
  if (isLoading) {
    return (
  ...
  const getIconByType = (type: string, templateType: string) => {
    ...
    if (templateType === "proposal" || templateType === "contract") return <FilePen className="h-4 w-4" />;
    return <FileText className="h-4 w-4" />;
  };

  const getColorByType = (type: string, templateType: string) => {
    if (templateType === "invoice") return "bg-blue-50 text-blue-500";
    if (templateType === "proposal" || templateType === "contract") return "bg-green-50 text-green-500";
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <a href="/files?filter=templates" className="text-primary text-xs font-medium hover:underline">View All</a>
      </div>
      
      <div className="space-y-3">
        {templates.length > 0 ? (
          templates.map((template) => (
            <div key={template.id} className="flex items-center p-2 hover:bg-gray-50 rounded-lg">
              <div className={`w-8 h-8 ${getColorByType(template.type, template.templateType || '')} rounded flex items-center justify-center mr-3`}>
