...
export default function AgentToolsPanel() {
  ...
  const [selectedTemplate, setSelectedTemplate] =
    useState<AgentToolTemplate | null>(null);
  ...
  const updateToolMutation = useMutation({
    mutationFn: async ({
      ...
      id: number;
      data: z.infer<typeof agentToolSchema>;
    }) => {
      const transformedData = {
        ...data,
        config: JSON.parse(data.config),
  ...
  const handleEditClick = (tool: AgentTool) => {
    ...
    const configStr = tool.config ? JSON.stringify(tool.config, null, 2) : "{}";
      ...
      tools?.some(
        (tool) =>
          ...
          (tool.config &&
            ...
            "templateName" in tool.config &&
            tool.config.templateName === templateName),
      ) || false
    );
  };
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="all" onValueChange={setActiveTab}>
        <TabsList className="mb-4 sm:mb-5 md:mb-6 flex flex-wrap h-auto gap-1.5 p-1 bg-muted/50 rounded-lg overflow-x-auto">
          <TabsTrigger
            className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-8 sm:h-9"
            value="all"
          >
            All Tools
          </TabsTrigger>
          <TabsTrigger
            className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-8 sm:h-9"
            value={TOOL_CATEGORIES.CONTENT_GENERATION}
          >
            Content Generation
          </TabsTrigger>
          <TabsTrigger
            className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-8 sm:h-9"
            value={TOOL_CATEGORIES.DATA_PROCESSING}
          >
            Data Processing
          </TabsTrigger>
          <TabsTrigger
            className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-8 sm:h-9"
            value={TOOL_CATEGORIES.COMMUNICATION}
          >
            Communication
          </TabsTrigger>
          <TabsTrigger
            className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-8 sm:h-9"
            value={TOOL_CATEGORIES.KNOWLEDGE}
          >
            Knowledge
          </TabsTrigger>
          <TabsTrigger
            className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-8 sm:h-9"
            value={TOOL_CATEGORIES.UTILITIES}
          >
            Utilities
          </TabsTrigger>
          <TabsTrigger
            className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-8 sm:h-9"
            value={TOOL_CATEGORIES.INTEGRATIONS}
          >
            Integrations
          </TabsTrigger>
          <TabsTrigger
            className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-8 sm:h-9"
            value={TOOL_CATEGORIES.CUSTOM}
          >
            Custom
          </TabsTrigger>
        </TabsList>
                ...
                {availableTemplates.length > 0 && (
                  <>
                    <div className="col-span-full mb-2 sm:mb-3 md:mb-4">
                      <h3 className="text-base sm:text-lg font-medium mb-0">
                        Available Templates
                      </h3>
                    </div>
                          ...
                          <CardHeader className="pb-2 px-3 sm:px-6 pt-3 sm:pt-6">
                            <div className="flex justify-between items-start">
                              ...
                              <Badge
                                ...
                                className="text-[10px] sm:text-xs h-5 sm:h-6 px-1.5 sm:px-2.5"
                              >
                  ...
                  <>
                    ...
                    {filteredTools.map((tool) => {
                      ...
                      return (
                        <Card
                          key={tool.id}
                          className={`${!tool.isActive ? "opacity-70" : ""} hover:shadow-md transition-shadow duration-200`}
                        >
                          <CardHeader className="pb-2 px-3 sm:px-6 pt-3 sm:pt-6">
                            <div className="flex justify-between items-start">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <CardTitle className="text-sm sm:text-base flex items-center gap-1 sm:gap-2 flex-wrap">
                                    ...
                                    {!tool.isActive && (
                                      <Badge
                                        variant="outline"
                                        className="text-[10px] sm:text-xs h-5 sm:h-6 px-1.5 sm:px-2.5 whitespace-nowrap"
                                      >
                                        Inactive
                                      </Badge>
                                    )}
                              ...
                              <DropdownMenu>
                                ...
                                >
                                  <DropdownMenuItem
                                    ...
                                    className="text-xs sm:text-sm py-1.5 h-8"
                                  >
                    ...
                    >
                      ...
                      <SelectContent className="max-h-[50vh] text-xs sm:text-sm">
                        ...
                        <SelectItem
                          ...
                          className="text-xs sm:text-sm flex items-center gap-2"
                        >
                          <BarChart className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Bar
                          Chart
                        </SelectItem>
                        <SelectItem
                          value="activity"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription className="text-xs">
                      Configuration depends on the tool type. For OpenAI tools,
