      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('subscription.chooseYourPlan')}</CardDescription>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="monthly">
              <TabsList className="grid w-full grid-cols-2 mb-4">
                <TabsTrigger value="monthly">{t('subscription.switchToMonthly')}</TabsTrigger>
                <TabsTrigger value="yearly">{t('subscription.switchToYearly')}</TabsTrigger>
              </TabsList>
              <TabsContent value="monthly">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-medium">{t('subscription.monthlyPrice')}</span>
                  <span className="text-2xl font-bold">199 SAR</span>
                </div>
              </TabsContent>
              <TabsContent value="yearly">
                <div className="flex justify-between items-center mb-2">
                  <span className="font-medium">{t('subscription.yearlyPrice')}</span>
                  <span className="text-2xl font-bold">1,999 SAR</span>
                </div>
                <div className="text-sm text-emerald-600">{t('subscription.saveWithYearly')}</div>
              </TabsContent>
            </Tabs>
          </CardContent>
          <CardFooter>
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
            <div className="p-4 border rounded-lg">
              <h3 className="font-semibold mb-2">{t('errors.somethingWentWrong')}</h3>
              <p className="text-sm text-muted-foreground">{t('errors.tryAgain')}</p>
            </div>
            
            <div className="p-4 border rounded-lg">
              <h3 className="font-semibold mb-2">{t('credentials.secureStorage')}</h3>
              <p className="text-sm text-muted-foreground">{t('files.dragAndDrop')} {t('files.or')} {t('files.browseFiles')}</p>
            </div>
          </div>
        </CardContent>
      </Card>
