...
export default function SubscriptionPage() {
  ...
  const createPaymentSession = useMutation({
    mutationFn: async (planId: number) => {
      const res = await apiRequest("POST", "/api/payments/create-session", {
        planId,
      });
  ...
  const filteredPlans = sortedPlans.filter(
    (plan) => plan.interval.toLowerCase() === planInterval,
  );
    ...
    <div className="container py-10 mx-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Unlock the full potential of Mirxa.io with our flexible subscription
          plans. Choose the plan that best suits your needs.
        </p>
      </div>
      ...
      {subscription && (
        <div className="mb-10">
          <Alert
            className={
              subscription.plan !== "free" ? "bg-primary/10" : undefined
            }
          >
      ...
      <div className="flex justify-center mb-8">
        <Tabs
          ...
          onValueChange={(v) => setPlanInterval(v as "monthly" | "yearly")}
        >
          ...
          >
            ...
            <CardFooter>
              <Button
                className="w-full"
                disabled={
                  isCurrentPlan(plan.name) || createPaymentSession.isPending
                }
